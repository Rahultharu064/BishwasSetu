...
function RegisterInner() {
  ...
  function useMyLocation() {
    if (!("geolocation" in navigator)) {
      toast(tr("auth.register.toastLocationUnavailable"), "error");
      return;
    }
    setLocating(true);
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        setLocating(false);
        toast(tr("auth.register.toastLocationCaptured"), "success");
      },
      () => {
        setLocating(false);
        toast(tr("auth.register.toastLocationFailed"), "error");
      },
  ...
  function validate() {
    ...
    if (form.name.trim().length < 2) e.name = tr("auth.register.errName");
    if (!form.email && !form.phone) e.email = tr("auth.register.errEmailOrPhone");
    if (form.email && !/^[^@]+@[^@]+\.[^@]+$/.test(form.email))
      e.email = tr("auth.register.errEmailInvalid");
    if (form.phone && !/^(\+977)?[9][6-9]\d{8}$/.test(form.phone))
      e.phone = tr("auth.register.errPhoneInvalid");
    if (form.password.length < 8) e.password = tr("auth.register.errPasswordLen");
    if (form.district.trim().length < 2) e.district = tr("auth.register.errDistrict");
    if (form.city.trim().length < 2) e.city = tr("auth.register.errCity");
    setErrors(e);
  ...
  async function submit(ev: React.FormEvent) {
    ...
    try {
      ...
      toast(tr("auth.register.toastAccountCreated"), "success");
      ...
      toast(err instanceof ApiError ? err.message : tr("auth.register.errFailed"), "error");
    } finally {
      setLoading(false);
    }
  }
        ...
        <div className="rounded-2xl border border-[#e7e7e1] bg-white shadow-2xl shadow-black/8 overflow-hidden">
          ...
          <div className="px-8 py-7">
            ...
            <div className="mb-5 text-center">
              <h2 className="text-xl font-bold text-[#1a1d1c]">{tr("auth.register.heading")}</h2>
              <p className="mt-1 text-sm text-[#6b7280]">{tr("auth.register.subtitle")}</p>
            </div>
            ...
            <div className="mb-5 grid grid-cols-2 gap-1.5 rounded-xl border border-[#e7e7e1] bg-[#f1f1ec] p-1">
              {(["CUSTOMER", "PROVIDER"] as const).map((r) => (
                ...
                >
                  ...
                  {r === "CUSTOMER" ? tr("auth.register.needService") : tr("auth.register.imProvider")}
                </button>
              ))}
            </div>
            ...
            <form onSubmit={submit} className="space-y-3.5" noValidate>
              ...
              <div>
                <label htmlFor="email" className="block text-sm font-semibold text-[#1a1d1c] mb-1.5">
                  {tr("auth.register.emailLabel")}{" "}
                  <span className="text-[#9ca39f] font-normal text-xs">{tr("auth.register.optional")}</span>
                </label>
                <div className="flex items-center overflow-hidden rounded-xl border border-[#e7e7e1] bg-[#fafaf7] transition-all duration-200 focus-within:border-[#0E7C5B] focus-within:ring-2 focus-within:ring-[#0E7C5B]/15 focus-within:bg-white">
                  <div className="flex items-center justify-center w-11 shrink-0 border-r border-[#e7e7e1]">
              ...
              <div className="rounded-xl border border-[#e7e7e1] bg-[#f1f1ec]/60 p-3.5">
                <div className="mb-2.5 flex items-center gap-2 text-sm font-semibold text-[#1a1d1c]">
                  ...
                  {role === "CUSTOMER" ? tr("auth.register.whereServiceCustomer") : tr("auth.register.whereServiceProvider")}
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label htmlFor="district" className="block text-xs font-medium text-[#6b7280] mb-1">
                      {tr("auth.register.district")}
                    </label>
                    <input
                      id="district"
                      value={form.district}
                      onChange={(e) => setForm({ ...form, district: e.target.value })}
                      placeholder={tr("auth.register.districtPlaceholder")}
                  ...
                  <div>
                    <label htmlFor="city" className="block text-xs font-medium text-[#6b7280] mb-1">
                      {tr("auth.register.city")}
                    </label>
                    <input
                      id="city"
                      value={form.city}
                      onChange={(e) => setForm({ ...form, city: e.target.value })}
                      placeholder={tr("auth.register.cityPlaceholder")}
                ...
                >
                  ...
                  {coords
                    ? tr("auth.register.locationCaptured")
                    : locating
                    ? tr("auth.register.gettingLocation")
                    : tr("auth.register.useMyLocation")}
                </button>
              </div>
              ...
              >
                ...
                {loading ? tr("auth.register.creating") : tr("auth.register.heading")}
              </button>
            </form>
            ...
            <div className="mt-4 flex items-center justify-center gap-1.5">
              ...
              <span className="text-xs text-[#6b7280]">{tr("auth.register.verifiedEscrow")}</span>
            </div>
            ...
            <div className="mt-4 flex items-center gap-3">
              ...
              <span className="text-xs text-[#9ca39f] whitespace-nowrap">
                {tr("auth.register.alreadyHaveAccount")}
              </span>
              <div className="flex-1 h-px bg-[#e7e7e1]" />
            </div>
